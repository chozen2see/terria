var protomaps = (() => {
  ...
  // src/attribute.ts
  var ColorAttr = class {
    constructor(c2, defaultValue = "black") {
      this.color = c2 || defaultValue;
      this.per_feature = typeof this.color == "function" && this.color.length == 2;
    }
    get(z2, f2) {
      if (typeof this.color == "function") {
        return this.color(z2, f2);
      } else {
        return this.color;
      }
    }
  ...
  var IconSymbolizer = class {
    ...
    place(layout, geom, feature) {
      let pt = geom[0];
      let a2 = new import_point_geometry3.default(geom[0][0].x, geom[0][0].y);
      let bbox = {
        minX: a2.x - 32,
        minY: a2.y - 32,
        maxX: a2.x + 32,
  ...
  var CenteredSymbolizer = class {
    ...
    place(layout, geom, feature) {
      let a2 = geom[0][0];
      let placed = this.symbolizer.place(layout, [[new import_point_geometry3.default(0, 0)]], feature);
  ...
  var OffsetSymbolizer = class {
    ...
    place(layout, geom, feature) {
      ...
      var origin = new import_point_geometry3.default(offset, -offset);
      let draw = (ctx) => {
      ...
      if (!layout.index.bboxCollides(bbox, layout.order))
        return [{ anchor, bboxes: [bbox], draw }];
      origin = new import_point_geometry3.default(-offset - fb.maxX, offset - fb.minY);
      bbox = getBbox(anchor, origin);
      if (!layout.index.bboxCollides(bbox, layout.order))
        return [{ anchor, bboxes: [bbox], draw }];
      origin = new import_point_geometry3.default(-offset - fb.maxX, -offset);
      bbox = getBbox(anchor, origin);
      if (!layout.index.bboxCollides(bbox, layout.order))
        return [{ anchor, bboxes: [bbox], draw }];
      origin = new import_point_geometry3.default(-offset - fb.maxX, offset - fb.minY);
  ...
  var PolygonLabelSymbolizer = class {
    ...
    place(layout, geom, feature) {
      ...
      let a2 = new import_point_geometry3.default(found[0], found[1]);
      let bbox = {
    ...
    for (var prepared_tile of prepared_tiles) {
      ...
      prepared_tile.data.forEach((features, layerName) => {
        ...
        for (var feature of features) {
          ...
          if (ps != 1) {
            geom = transformGeom(geom, ps, new import_point_geometry4.default(0, 0));
          }
          if (feature.geomType == GeomType.Point) {
  ...
  var instancedProject = (origin, display_zoom) => {
    return (latlng) => {
      ...
      let normalized = new import_point_geometry5.default((projected.x + MAXCOORD2) / (MAXCOORD2 * 2), 1 - (projected.y + MAXCOORD2) / (MAXCOORD2 * 2));
  ...
  var instancedUnproject = (origin, display_zoom) => {
    return (point) => {
      console.log(point);
      let normalized = new import_point_geometry5.default(point.x, point.y).add(origin).div((1 << display_zoom) * 256);
      let projected = new import_point_geometry5.default(normalized.x * (MAXCOORD2 * 2) - MAXCOORD2, (1 - normalized.y) * (MAXCOORD2 * 2) - MAXCOORD2);
  ...
  var Static = class {
    ...
    drawContext(ctx, width, height, latlng, display_zoom) {
      return __async(this, null, function* () {
        ...
        let origin = normalized_center.clone().mult(Math.pow(2, display_zoom) * 256).sub(new import_point_geometry5.default(width / 2, height / 2));
