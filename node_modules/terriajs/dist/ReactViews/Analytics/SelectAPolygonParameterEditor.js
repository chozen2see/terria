...
const SelectAPolygonParameterEditor = createReactClass({
    ...
    render() {
        ...
        return (React.createElement("div", null,
            React.createElement("input", { className: Styles.field, type: "text", value: this.state.value }),
            React.createElement("button", { type: "button", onClick: this.selectExistingPolygonOnMap, className: Styles.btnSelector }, t("analytics.selectExistingPolygon"))));
    }
});
/**
...
export function selectOnMap(terria, viewState, parameter) {
    ...
    reaction(() => pickPolygonMode.pickedFeatures, async (pickedFeatures, reaction) => {
        ...
        if (result.error) {
            ...
            runInAction(() => {
                parameter.setValue(CommonStrata.user, catalogItems.map(item => item.readyData));
                terria.mapInteractionModeStack.pop();
                viewState.openAddData();
            });
        }
