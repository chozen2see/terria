    ...
    if (resultMap.get(model.uniqueId) === undefined) {
      ...
      // saveModelToJson(modelToSave, {
        ...
        autorun(reaction => {
          ...
          console.log("CatalogSearchProvider", searchString);
  ...
  protected async doSearch(
    ...
    try {
      if (this.terria.catalogIndex) {
        console.log("CatalogSearchProvider: terria catalogIndex exists");
        const results = await this.terria.catalogIndex?.search(searchText);
        runInAction(() => (searchResults.results = results));
      } else {
        console.log("CatalogSearchProvider: use loadAndSearchCatalogRecursively instead")
